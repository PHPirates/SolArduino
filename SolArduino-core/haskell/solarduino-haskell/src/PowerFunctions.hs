...
directPower ::
            ...
            -> Double -- ^ The angle the solar panels have with the sun, in radians
            -> Double -- ^ The power the panels receive from the sun in W/m^2
directPower n theta
    ...
    | otherwise
     ...
     =
        let i = 1367 * (1 + 0.034 * cos (360 * fromIntegral n / 365.25))
            ...
            k = 0.249 + 0.081 * 2.5 ^ 2
            -- Use the ** power because from a cosine results in a Double, not a fraction
